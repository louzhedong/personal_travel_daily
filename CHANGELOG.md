...
- 新增 10 份中英双语专项技术文档，覆盖日程化行前规划、整理工作台、智能相册、标签治理、攻略质量、私密分享、本地归档、旅行预算、地图回放故事和主动提醒。
  Added 10 bilingual feature technical docs covering scheduled planning, organization, smart albums, tag governance, guide quality, private sharing, local archive packages, travel expenses, map replay stories, and proactive reminders.
- 同步更新 README、文档导航、项目总览、前端架构、后端架构和 App API 契约，确保 Phase 12 交付在入口文档中可发现。
  Updated the README, docs index, project overview, frontend architecture, backend architecture, and App API contract so the Phase 12 delivery is discoverable from entry docs.

### Changed / 变更

- 将 11 个 roadmap 条目迁移到“已完成里程碑”，并清空当前活动推荐队列，避免已交付能力继续显示为待办。
  Moved the 11 roadmap items into Completed Milestones and cleared the active recommendation queue so shipped capabilities no longer appear as pending work.

### Verified / 已验证

- `git diff --check`

### PR 待定 / TBD `feat: 本地归档纪念包 / Add local archive keepsake packages`

### Added / 新增

- 新增浏览器端本地归档包导出，行程故事、年度回顾和胶囊详情均可下载 ZIP。
  Added browser-side local archive package exports, allowing Trip Story, Annual Review, and Memory Capsule detail pages to download ZIP files.
- 新增 manifest 与无压缩 ZIP helper，归档包包含 `manifest.json`、内容 JSON、Markdown 摘要、SVG 预览和图片 URL 清单。
  Added manifest and uncompressed ZIP helpers; packages include `manifest.json`, content JSON, Markdown summary, SVG preview, and image URL list.
...
### PR 待定 / TBD `feat: 日程化行前规划 / Add scheduled trip planning`

### Added / 新增

- 新增行前规划日程 API：`GET /api/trips/:id/planning/schedule`、`PATCH /api/trips/:id/planning/items/:itemId/schedule`、`POST /api/trips/:id/planning/schedule/import-wishlist`。
  Added trip-planning schedule APIs: `GET /api/trips/:id/planning/schedule`, `PATCH /api/trips/:id/planning/items/:itemId/schedule`, and `POST /api/trips/:id/planning/schedule/import-wishlist`.
- 新增行程详情内“日程视图”，支持未排期池、按天排期、愿望地图导入到指定日期，以及当天辅助清单提示。
  Added a trip-detail schedule view with an unscheduled pool, day-by-day scheduling, wishlist import into a selected date, and day-level checklist hints.
- 新增前端日程模型、日程看板样式，以及后端 service / route 与前端页面定向测试。
  Added the frontend schedule model, schedule-board styles, and focused backend service/route plus frontend page tests.

### Verified / 已验证

- `npm run test -- server/__tests__/appApiRoutes.trips.spec.ts server/__tests__/tripPlanningScheduleService.spec.ts src/modules/trips/__tests__/tripPlanningCalendarModel.spec.ts src/modules/__tests__/TripDetailPage.spec.tsx`
