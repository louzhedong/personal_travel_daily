...
- **视觉 Shell 收口 / Visual Shell**：所有新页面外层必须使用 `--page-frame`（1180px 窄页框，与旅行杂志默认内容页一致）容器，shell 选择器（`*-shell`）统一在 `src/styles/visual-system.css` 注册。仅首页 / 工作台类宽幅页面（admin / stats / atlas / journey / live-trip 等）保留 `--page-frame-wide`（1320px）。新页面禁止内联 `max-width`，禁止默认走 wide-frame——否则在常规视口下卡片会贴边、缺少视觉边距。
- **卡片内边距 / Card Inner Padding**：`.card` 仅提供背景 / 描边 / 阴影，**没有默认 padding**；任何 `<section className="card">` 都必须叠 `panel-card`（即写为 `className="card panel-card"`），由 `panel-card` 统一吃 `--space-card-y` / `--space-card-x` 内边距，否则卡片内文字会紧贴边框。/ `.card` only provides background / border / shadow with **no default padding**. Every `<section className="card">` MUST be combined with `panel-card` (write `className="card panel-card"`) so that `panel-card` applies the unified `--space-card-y` / `--space-card-x` inner padding — otherwise card content sticks to the borders.
