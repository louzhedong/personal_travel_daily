...
- `AppRoute` 联合类型枚举当前所有顶层路由：`/`、`/login`、`/register`、`/settings`、`/admin`、`/atlas`、`/organize`、`/tags`、`/stats`、`/achievements`、`/reminders`、`/capsules`、`/capsules/:id`、`/photos`、`/share/:token`、`/replay/:targetType/:targetId`、`/companions/:id/memories`、`/trips/:id`、`/trips/:id/story`、`/trips/:id/checklist`、`/yearbook/:year`。
  `AppRoute` enumerates all top-level routes including `/`, auth routes, settings, admin, Atlas, organization, tags, stats, achievements, reminders, capsules, photos, public shares, replay stories, companion memories, trip detail/story/checklist, and annual review.
...
## 13. Roadmap 功能页面 / Roadmap Feature Surfaces

Phase 12 收口后，前端新增能力继续遵循 `router -> routeRenderers -> module page -> model/API/style` 的路径。
After the Phase 12 closure, new frontend capabilities continue to follow `router -> routeRenderers -> module page -> model/API/style`.

- `/organize` 使用 `OrganizationWorkbenchPage` 与 `organizationWorkbenchModel`，所有批量整理先预览再确认。
  `/organize` uses `OrganizationWorkbenchPage` and `organizationWorkbenchModel`, with preview-before-apply for every bulk action.
- `/tags` 使用 `TagGovernancePage`，并通过 `tagVocabularyApi` 与 marker 表单、列表、统计和 Atlas 共享词表。
  `/tags` uses `TagGovernancePage` and shares vocabulary through `tagVocabularyApi` across marker forms, lists, stats, and Atlas.
- `/reminders` 使用 `ReminderCenterPage` 与 `reminderModel`，只承担低打扰应用内提醒，不做外部推送。
  `/reminders` uses `ReminderCenterPage` and `reminderModel` for low-noise in-app reminders only, with no external delivery.
- `/replay/:targetType/:targetId` 使用 `MapReplayStoryPage`，复用 Atlas / 回放表达并保持本地 SVG 导出。
  `/replay/:targetType/:targetId` uses `MapReplayStoryPage`, reusing Atlas/replay presentation and local SVG export.
- `/share/:token` 使用 `PublicSharePage`，剥离登录态导航、编辑操作和账号内管理信息。
  `/share/:token` uses `PublicSharePage`, removing authenticated navigation, editing controls, and account-management details.

Summary: The new pages preserve the magazine-style layout, typed API clients, module-scoped models, and feature CSS barrels.
