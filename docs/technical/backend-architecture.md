...
## 11. Roadmap 领域服务 / Roadmap Domain Services

Phase 12 收口后，后端新增领域继续遵循 `routes -> schemas -> services -> repositories -> serializers -> dto` 分层，并保持 DTO 目录化与 barrel 兼容。
After the Phase 12 closure, new backend domains continue to follow `routes -> schemas -> services -> repositories -> serializers -> dto`, while keeping domain DTO folders and compatibility barrels.

- `tripPlanningScheduleService.ts` 承接行前规划日程聚合、排期变更和愿望导入到指定日期。
  `tripPlanningScheduleService.ts` owns planning schedule aggregation, schedule updates, and wishlist import into a selected day.
- `organizationWorkbenchService.ts` 聚合整理建议，并以 preview/apply 二阶段执行批量整理。
  `organizationWorkbenchService.ts` aggregates cleanup suggestions and executes batch organization through preview/apply phases.
- `photoAlbumService.ts`、`tagVocabularyService.ts`、`guideQualityService.ts` 分别管理封面策展、账号级词表和攻略质量评分。
  `photoAlbumService.ts`, `tagVocabularyService.ts`, and `guideQualityService.ts` manage cover curation, account vocabulary, and guide quality scoring.
- `shareLinkService.ts` 使用 hash-only token、访问审计、撤销、过期和可选密码保护公开只读访问。
  `shareLinkService.ts` uses hash-only tokens, access audit logs, revocation, expiry, and optional password protection for public read-only access.
- `expenseService.ts`、`mapReplayStoryService.ts`、`reminderService.ts` 分别承接消费洞察、回放故事聚合和应用内提醒。
  `expenseService.ts`, `mapReplayStoryService.ts`, and `reminderService.ts` own expense insights, replay-story aggregation, and in-app reminders.

Summary: Each roadmap domain keeps thin Fastify routes, typed schemas, account-scoped services, Prisma-backed repositories where needed, and serializers for DTO shaping.
