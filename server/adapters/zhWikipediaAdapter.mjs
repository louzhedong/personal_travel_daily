...
const WIKI_BASE = 'https://zh.wikipedia.org/wiki/';
...
function buildEntry(title, description = '') {
  return createCatalogEntry({
    id: `zh-wikipedia-${encodeURIComponent(title)}`,
    adapterId: 'zh-wikipedia',
    scope: 'all',
    title: `${title} | 维基百科`,
    summary: description || `${title} 的中文目的地参考信息`,
    sourceName: '维基百科',
    sourceUrl: buildPageUrl(title),
    destinationLabel: title,
    tags: ['中文参考', '维基百科'],
    searchableText: `${title} 旅行 攻略 目的地 维基百科 中文`,
  });
}
...
export const zhWikipediaAdapter = {
  id: 'zh-wikipedia',
  entries: [],
